...
plot_fits <- function(stan_fit, lambda_ = -1, gamma_ = -1)
{
  ...
  cis <- dplyr::group_by(df, variable) %>% dplyr::summarise(lower95 = quantile(value, 0.025), upper95 = quantile(value, 0.975),
  ...
  facet_names <- c(lambda="Force of infection", gamma="Rate of recovery", R0="R0")
...
optimiseRobustExperimentalDesign <- function(stan_fit, delta0 = 10, num_obs = 12, num_samples = 10000)
{
  ...
  powell(delta0, fn = robustExperimentalDesign, control = cntrl, stan_fit = stan_fit, num_obs = num_obs, num_samples = num_samples)
}