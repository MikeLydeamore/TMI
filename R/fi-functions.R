...
FI <- function(gamma, lambda, delta, n_obs, i0=0.316)
{
  ...
  for (i in 1:n_obs)
  {
    top_left <- top_left + sum((x_0 %*% (P%^%i))[,1])*( (1-phi_s)*.one_minus_phi_s_lambda_lambda(gamma,lambda,delta) + phi_s*.phi_s_lambda_lambda(gamma,lambda,delta) ) +
      sum((x_0%*% (P%^%i))[,2])*( (1-phi_i)*.one_minus_phi_i_lambda_lambda(gamma,lambda,delta) + phi_i*.phi_i_lambda_lambda(gamma,lambda,delta) )
    
    top_right <- top_right + sum((x_0 %*% (P%^%i))[,1])*( (1-phi_s)*.one_minus_phi_s_gamma_lambda(gamma,lambda,delta) + phi_s*.phi_s_lambda_gamma(gamma,lambda,delta) ) +
      sum((x_0%*% (P%^%i))[,2])*( (1-phi_i)*.one_minus_phi_i_lambda_gamma(gamma,lambda,delta) + phi_i*.phi_i_lambda_gamma(gamma,lambda,delta) )

    bottom_left <- bottom_left + sum((x_0 %*% (P%^%i))[,1])*( (1-phi_s)*.one_minus_phi_s_gamma_lambda(gamma,lambda,delta) + phi_s*.phi_s_lambda_gamma(gamma,lambda,delta) ) +
      sum((x_0%*% (P%^%i))[,2])*( (1-phi_i)*.one_minus_phi_i_lambda_gamma(gamma,lambda,delta) + phi_i*.phi_i_lambda_gamma(gamma,lambda,delta) )

    bottom_right <- bottom_right + sum((x_0 %*% (P%^%i))[,1])*( (1-phi_s)*.one_minus_phi_s_gamma_gamma(gamma,lambda,delta) + phi_s*.phi_s_gamma_gamma(gamma,lambda,delta) ) +
